...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
  ...
  if (empty($title)) {
    $messages[] = ['text' => 'Title is required', 'colour' => 'text-red-500'];
    $submitted = false;
  }

  if (empty($description)) {
    $messages[] = ['text' => 'Description is required', 'colour' => 'text-red-500'];
    $submitted = false;
  }
  ...
  if ($file['error'] === UPLOAD_ERR_OK) {
    ...
    if (in_array($fileExtension, $allowedExtensions)) {
      ...
      if (move_uploaded_file($file['tmp_name'], $uploadDir .  $filename)) {
        $messages[] = ['text' => 'File uploaded successfully.', 'colour' => 'text-green-500'];
        $submitted = true;
      } else {
        $messages[] = ['text' => 'File upload error', 'colour' => 'text-red-500'];
      }
    } else {
      $messages[] = ['text' => 'File type error', 'colour' => 'text-red-500'];
    }
  }
}
?>
